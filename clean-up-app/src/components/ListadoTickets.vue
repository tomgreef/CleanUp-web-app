<template>
    <section class="section">
        <div v-if="tickets.length != 0">
            <Ticket v-for="ticket in tickets" :key="ticket"/>
        </div>

        <div v-else class="is-centered">
            <b-message title="Sin incidencias" type="is-info" :closable="false">
                <div class="has-text-centered">
                    <h1 class="title">
                        Aún no has creado ninguna incidencia 
                    </h1>

                    <svg style="width:20em" viewBox="0 0 24 24">
                        <path fill="currentColor" d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M7,9.5C7,8.7 7.7,8 8.5,8C9.3,8 10,8.7 10,9.5C10,10.3 9.3,11 8.5,11C7.7,11 7,10.3 7,9.5M14.77,17.23C14.32,16.5 13.25,16 12,16C10.75,16 9.68,16.5 9.23,17.23L7.81,15.81C8.71,14.72 10.25,14 12,14C13.75,14 15.29,14.72 16.19,15.81L14.77,17.23M15.5,11C14.7,11 14,10.3 14,9.5C14,8.7 14.7,8 15.5,8C16.3,8 17,8.7 17,9.5C17,10.3 16.3,11 15.5,11Z" />
                    </svg>
                    <br><br>
                    <h2 class="subtitle">
                        Prueba crear tu primera incidencia ahora
                    </h2>

                    <b-button tag="router-link"
                        to="/CreateTicket"
                        type="is-link">
                        Crear primera incidencia
                    </b-button>   
                </div>
            </b-message>
        </div>
    </section>
</template>

<script>
    import Ticket from '@/components/Ticket';
    export default {
        data() {
            return {
                tickets: []
            }
        },
		components: {
            Ticket
		}
	};
</script>